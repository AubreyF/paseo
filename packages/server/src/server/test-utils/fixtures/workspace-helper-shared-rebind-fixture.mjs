...
const [helper, counterPath, sharedRoot, ...args] = process.argv.slice(2);
if (!helper || !counterPath || !sharedRoot) {
  throw new Error("helper, counter path, and shared root are required");
}

const rootIndex = args.indexOf("--root");
const observedRoot = rootIndex < 0 ? null : args[rootIndex + 1];
if (args[0] !== "watch" || observedRoot !== sharedRoot) {
  await forward();
} else {
  const launch = Number(await readFile(counterPath, "utf8").catch(() => "0")) + 1;
  await writeFile(counterPath, String(launch));
  if (launch === 2) await failSecondSubscription();
  else await forward();
}
