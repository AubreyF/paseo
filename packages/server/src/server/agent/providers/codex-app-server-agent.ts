...
export class CodexAppServerAgentClient implements AgentClient {
  ...
  private resolveAutoReviewEnabled(workspace?: ProviderWorkspace): Promise<boolean> {
    if (workspace) {
  ...
  async fetchCatalog(options: FetchCatalogOptions): Promise<ProviderCatalog> {
    ...
    const [models, autoReviewEnabled] = await Promise.all([
      this.fetchModelsFromAppServer(workspace),
      this.resolveAutoReviewEnabled(workspace),
    ]);
    return {
      models,
